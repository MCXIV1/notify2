...
window.addEventListener('DOMContentLoaded', () => {
    try {
        electron_1.ipcRenderer.sendToHost('mm-ready');
    }
    catch { }
});
let currentUnread = 0;
function sendUnread() {
    try {
        electron_1.ipcRenderer.sendToHost('mm-unread', currentUnread);
    }
    catch { }
}
electron_1.contextBridge.exposeInMainWorld('rambox', {
...
(function setupTitleWatcher() {
    ...
    const parse = () => {
        ...
        if (m) {
            ...
            if (!Number.isNaN(n))
                window.rambox?.setUnreadCount(n);
        }
    };
    if (titleEl) {
